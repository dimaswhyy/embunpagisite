...
<div class="block w-6/12">
  <h4 class="mb-8 text-xl font-semibold">Update Facilities Category</h4>
  <form action="{{ route('update_facilities_category') }}" method="post" class="block">
    @csrf
    <input type="hidden" name="id" value="{{ $data->translation[0]->id }}" />
    ...
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-full overflow-hidden bg-white">
        <input id="title-input" type="text" name="title" class="border-0 rounded-lg px-3 py-2 text-sm w-full bg-none" placeholder="type title" value="{{ $data->translation[0]->title }}" />
      </div>
    </div>
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-2/12 overflow-hidden">
        <input id="num-order-input" type="number" name="num_order" class="border-0 rounded-lg px-3 py-2 text-sm w-full bg-none" min="1" value="{{ $data->num_order }}" />
      </div>
    </div>
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-fit overflow-hidden bg-white px-3 py-2 ">
        <select id="status-input" name="status" class="border-0 rounded-lg text-sm w-fit pr-5">
          <option value="1" @if ($data->status === 1) selected="selected" @endif>Publish</option>
          <option value="0" @if ($data->status === 0) selected="selected" @endif>Draft</option>
        </select>
      </div>
    </div>
    <div class="block mb-5">
