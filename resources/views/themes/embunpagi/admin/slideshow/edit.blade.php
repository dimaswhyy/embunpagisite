...
<div class="block w-6/12">
  <h4 class="mb-8 text-xl font-semibold">Update Slideshow</h4>
  <form action="{{ route('slideshow_update') }}" method="post" class="block">
    @csrf
    <input type="hidden" name="id" value="{{ $slideshow->translation[0]->id }}" />
    ...
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-full overflow-hidden bg-white">
        <input id="title-input" type="text" name="title" class="border-0 rounded-lg px-3 py-2 text-sm w-full bg-none" placeholder="type name" value="{{ $slideshow->translation[0]->title }}" />
      </div>
    </div>
    <div class="block mb-5">
      ...
      <div id="add-single-image" class="block">
        <img id="img-preview-upload" src="{{ asset('storage') }}/{{ $slideshow->translation[0]->image }}" filename="{{ $slideshow->translation[0]->image }}" alt="{{ $slideshow->translation[0]->image }}" class="img-preview-upload mb-3" />
        <button 
          type="button" 
    ...
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-fit overflow-hidden bg-white px-3 py-2 ">
        <select id="status-input" name="status" class="border-0 rounded-lg text-sm w-fit pr-5">
          <option value="1" @if($slideshow->status === 1) selected="selected" @endif>Publish</option>
          <option value="0" @if($slideshow->status === 0) selected="selected" @endif>Draft</option>
        </select>
      </div>
    </div>
    <div class="block mb-5">
