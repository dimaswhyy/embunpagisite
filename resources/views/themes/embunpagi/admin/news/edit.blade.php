...
<div class="block w-6/12">
  <h4 class="mb-8 text-xl font-semibold">Update Publication</h4>
  <form action="{{ route('update_publication') }}" method="post" class="block">
    @csrf
    <input type="hidden" name="id" value="{{ $news->translation[0]->id }}" />
    ...
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-full overflow-hidden bg-white">
        <input id="title-input" type="text" name="title" class="border-0 rounded-lg px-3 py-2 text-sm w-full bg-none" placeholder="type title" value="{{ $news->translation[0]->title }}" />
      </div>
    </div>
    <div class="block mb-5">
    ...
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-fit overflow-hidden bg-white px-3 py-2 ">
        <select id="category-input" name="news_category_id" class="border-0 rounded-lg text-sm w-fit pr-5">
          <option value="">Select Category</option>
          @foreach ($category as $item)
          <option value="{{ $item->id }}" @if ($news->news_category_id === $item->id) selected="selected" @endif>{{ $item->translation[0]->title }}</option>
          @endforeach
        </select>
      </div>
    </div>
    <div class="block mb-5">
      <label for="keyword-input" class="font-semibold text-sm text-gray-600 pb-1 block">Keyword</label>
      <div class="border rounded-lg mt-1 mb-5 w-full overflow-hidden bg-white">
        <input id="keyword-input" type="text" name="keyword" class="border-0 rounded-lg px-3 py-2 text-sm w-full bg-none" placeholder="type keyword" value="{{ $news->translation[0]->keyword }}" />
      </div>
    </div>
    <div class="block mb-5">
      ...
      <div id="add-single-image" class="block">
        <img id="img-preview-upload" src="{{ asset('storage') }}/{{ $news->translation[0]->image }}" filename="" alt="" class="img-preview-upload mb-3" />
        <button 
          type="button" 
        ...
        >Pilih File</button>
        <input id="image-input" type="hidden" name="image" value="{{ $news->translation[0]->image }}" />
      </div>
    </div>
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-fit overflow-hidden bg-white px-3 py-2 ">
        <select id="status-input" name="status" class="border-0 rounded-lg text-sm w-fit pr-5">
          ...
          <option value="draft" @if ($news->status !== 'publish') selected="selected" @endif>Draft</option>
        </select>
      </div>
    </div>
    <div class="block mb-5">
