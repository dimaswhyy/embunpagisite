...
<div class="block w-6/12">
  <h4 class="mb-8 text-xl font-semibold">Add Publication</h4>
  <form action="{{ route('publication_store') }}" method="post" class="block">
    @csrf
    ...
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-full overflow-hidden bg-white">
        <textarea id="description-input" name="description" class="border-0 rounded-lg px-3 py-2 text-sm w-full bg-none tinymce-textarea" placeholder="type description"></textarea>
      </div>
    </div>
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-fit overflow-hidden bg-white px-3 py-2 ">
        <select id="category-input" name="news_category_id" class="border-0 rounded-lg text-sm w-fit pr-5">
          ...
          <option value="{{ $item->id }}">{{ $item->translation[0]->title }}</option>
          @endforeach
        </select>
      </div>
    </div>
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-full overflow-hidden bg-white">
        <input id="keyword-input" type="text" name="keyword" class="border-0 rounded-lg px-3 py-2 text-sm w-full bg-none" placeholder="type keyword" />
      </div>
    </div>
    <div class="block mb-5">
      ...
      <div id="add-single-image" class="block">
        <img id="img-preview-upload" src="" filename="" alt="" class="img-preview-upload mb-3" />
        <button 
          type="button" 
