...
<div class="bg-white px-4 py-6 shadow sm:rounded-lg sm:p-6">
  <div class="flex justify-between items-center">
    <h4 class="font-bold text-xl text-blue">Testimonials</h4>
    <a href="{{ route('testimonials_add') }}" class="transition duration-200 bg-blue hover:bg-blue-600 focus:bg-blue-700 focus:shadow-sm focus:ring-4 focus:ring-blue-500 focus:ring-opacity-50 text-white px-5 py-2 rounded-lg text-sm shadow-sm hover:shadow-md font-semibold text-center">Add Testimonial</a>
  </div>
  ...
  <div class="py-5">
    <table class="w-full my-5">
      <thead>
        <th class="text-sm font-semibold border-b border-t p-3 text-left">Photo</th>
        <th class="text-sm font-semibold border-b border-t p-3 text-left">Name</th> 
        <th class="text-sm font-semibold border-b border-t p-3 text-left">Generation</th>
      ...
      <tbody>
        ...
        <tr id="data-id-{{ $item->translation[0]->id }}">
          ...
          <td class="text-sm border-b p-3">{{ $item->translation[0]->subtitle }}</td>
          ...
          <td class="text-sm border-b p-3 whitespace-nowrap">
            <a href="{{ route('testimonials_edit', $item->id) }}" class="text-blue">Edit</a>&nbsp;
            <a href="javascript:void(0)" class="text-red-700 btn-delete" data-id="{{ $item->translation[0]->id }}" path-delete="{{ route('delete_module_content') }}">Delete</a>
          </td>
        </tr>
        @endforeach
      </tbody>
