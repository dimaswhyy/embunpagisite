...
<div class="block w-6/12">
  <h4 class="mb-8 text-xl font-semibold">Add Why Section Content</h4>
  <form action="{{ route('why_section_store') }}" method="post" class="block">
    @csrf
    <input type="hidden" name="id" value="{{ $whySection->translation[0]->id }}" />
    <input type="hidden" name="type" value="why-section-content" />
    ...
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-full overflow-hidden bg-white">
        <input id="title-input" type="text" name="title" class="border-0 rounded-lg px-3 py-2 text-sm w-full bg-none" placeholder="type title" value="{{ $whySection->translation[0]->title }}" />
      </div>
    </div>
    <div class="block mb-5">
      <label for="description-input" class="font-semibold text-sm text-gray-600 pb-1 block">Description</label>
      <div class="border rounded-lg mt-1 mb-5 w-full overflow-hidden bg-white">
        <textarea id="description-input" name="description" class="border-0 rounded-lg px-3 py-2 text-sm w-full bg-none tinymce-textarea" placeholder="type description">{{ $whySection->translation[0]->description }}</textarea>
      </div>
    </div>
    <div class="block mb-5">
