...
<div class="block w-6/12">
  <h4 class="mb-8 text-xl font-semibold">Add Page</h4>
  <form action="{{ route('page_store') }}" method="post" class="block">
    @csrf
    ...
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-full overflow-hidden bg-white">
        <input id="name-input" type="text" name="title" class="border-0 rounded-lg px-3 py-2 text-sm w-full bg-none" placeholder="type title" />
      </div>
    </div>
    <div class="block mb-5">
      <label for="description-input" class="font-semibold text-sm text-gray-600 pb-1 block">Description</label>
      <div class="border rounded-lg mt-1 mb-5 w-full overflow-hidden bg-white">
        <textarea id="description-input" name="description" class="border-0 rounded-lg px-3 py-2 text-sm w-full bg-none" placeholder="type description"></textarea>
      </div>
    </div>
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-full overflow-hidden bg-white px-3 py-2 ">
        <select id="parent-input" name="parent" class="border-0 rounded-lg text-sm w-full">
          ...
          @include('admin.pages-option-list', ['dataPages' => $dataPages, 'selected' => '', 'prefix' => ''])
        </select>
      </div>
    </div>
    <div class="block mb-5">
    ...
    <div class="block mb-5">
      ...
      <div class="border rounded-lg mt-1 mb-5 w-2/12 overflow-hidden">
        <input id="num-order-input" type="number" name="order_num" class="border-0 rounded-lg px-3 py-2 text-sm w-full bg-none" min="1" value="1" />
      </div>
    </div>
    <div class="block mb-5">
